...
5. **审批单次生效**：AI 或用户对越界工具调用的批准只对当前调用有效。桌面端显式选择的本地附件
   采用独立、可审计的会话分支授权，不复用工具审批，也不扩展到附件父目录或其他会话。
...
每次发送附件时，主进程生成随机 `requestId`，将 canonical 文件清单写入当前 session branch 的
...
3. 文件路径来自 custom entry，而不是 renderer 或消息正文。

匹配后的 canonical 文件路径作为 `SandboxPolicy.allowWrite` 传给当前 Harness；Core 的既有规则会
同时允许读取该精确路径。授权随 branch 历史继承，因此 fork 在分叉点之后保留已有附件授权，但
其他会话和工作区不继承。孤立、损坏或用户伪造的附件块不会扩权。
