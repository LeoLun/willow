...
- `workspace` 主 tab 已补齐工作空间名称与 `AGENTS.md` 编辑保存入口
...
1. `workspace` 右栏中的 `AGENTS.md` 设置到底写入哪里。
   当前 OpenSpec 只要求存在 `AGENTS.md` 设置区和保存入口，但未明确以下事实：
   - 是否读写 `<workspace.path>/AGENTS.md`
   - 是否读写应用自己的工作空间配置表
   - 是否当前阶段只展示占位与只读内容
...
5. 切片 E：最后做 `workspace` 设置面板，因为它依赖 `AGENTS.md` 存储语义确认
6. 切片 F：统一做状态和视觉收口
...
- `workspace` 右栏中的 `AGENTS.md` 存储语义会在实现前被补充明确

## 9. 下一步

进入 `workflow-implement`，按本计划的切片顺序实施；如遇到 `AGENTS.md` 存储语义或文件树过滤规则无法确认，先回到 `workflow-spec` 补足要求，再继续编码。
