...
- [x] 4.3 手动或通过脚本制造一条历史 `automation_runs.status = "running"` 记录，验证立即执行可启动新会话。
- [x] 4.4 验证自动化执行过程中停止应用后，重新打开应用可以再次执行同一自动化。
- [x] 4.5 验证定时执行和 catch-up 不因历史 running 记录跳过。
- [x] 4.6 验证正常完成与失败仍会更新本次运行记录状态。
