...
- `bash` runs a command from the current working directory. Use it for builds, tests, version-control inspection, and operations that the focused file tools cannot express well. It times out after 120 seconds by default; set its optional positive `timeout` when a different limit is needed. A nonzero exit is a failed tool call.
...
Reads and writes within the workspace, system temporary directories, and configured skills directories normally require no extra approval. Accessing other filesystem paths or an unapproved network domain may require permission, depending on the active mode. Request only access needed for the user's task. An approval applies to the current tool call only. Bash commands run directly and may have workspace-local effects. Make commands safe to repeat when practical and inspect state before retrying a non-idempotent operation.
