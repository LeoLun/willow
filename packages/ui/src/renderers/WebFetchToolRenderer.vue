...
const metaItems = computed(() => {
  ...
  if (requestUrl.value) items.push({ label: i18n("URL"), value: requestUrl.value });
  if (details.value.title) items.push({ label: i18n("Title"), value: details.value.title });
  if (details.value.contentType)
    items.push({ label: i18n("Type"), value: details.value.contentType });
  if (parsedParams.value.format || details.value.returnedFormat) {
    items.push({
      label: i18n("Format"),
      value: `${parsedParams.value.format || "markdown"} -> ${details.value.returnedFormat || parsedParams.value.format || "markdown"}`,
    });
  }
  if (typeof details.value.outputLength === "number") {
    items.push({
      label: i18n("Length"),
      value: `${details.value.outputLength} ${i18n("Characters")}`,
    });
  }
  if (typeof details.value.wasRetried === "boolean") {
    items.push({
      label: i18n("Retry"),
      value: details.value.wasRetried ? i18n("Yes") : i18n("No"),
    });
  }
  return items;
});
