...
import { STAGE_GIT_CHANGES } from "@shared/constants";
import { Injectable, IPC } from "@willow/poetry";
import { GitReviewController } from "./git-review.controller";

@Injectable()
export class StageGitChangesController extends GitReviewController<
  UpdateGitReviewIndexRequest,
  UpdateGitReviewIndexResponse
> {
  ...
  async run(
    ...
    try {
      await this.gitReviewService.stage(request.workspaceId, request.paths);
