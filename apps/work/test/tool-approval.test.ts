...
import { TOOL_APPROVAL_EVENT } from "../src/shared/constants";
...
describe("ToolApprovalService", () => {
  it("persists requests before serializing and resolving each decision once", async () => {
    const { appendCustomEntry, sendEvent, service } = createApprovalServices();
    const request = {
      toolCallId: "call",
    ...
    const firstPayload = sendEvent.mock.calls[0]![1];
    ...
    await vi.waitFor(() => expect(sendEvent).toHaveBeenCalledTimes(2));
    const secondPayload = sendEvent.mock.calls[1]![1];
